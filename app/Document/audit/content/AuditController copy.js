	...
	function AuditFormController($scope, $http, $filter,$stateParams,$state,FormBuilderManager)
		{
			 ...
			 /*
			 $scope.calculateFieldTotal=function(obj,fields)
			 {
				 var total=0;
				 for(var i in fields)
				 {
					 ...
					 if(fld)
					 {
						 total+=$scope.getNumberValue(fld.value);
					 }
				 }
				 obj.total.value=total;
				 obj.difference.value=$scope.getNumberValue(obj.balance.value)-total;
				 return total;
			 }
			 
			 $scope.calculateFormTotal=function()
			 {
				 var sales=$scope.formManager.fieldValues.sales_summary;

				 var declared_income=$scope.formManager.fieldValues.declared_income;
				 
				 $scope.calculateFieldTotal(sales,["cash","manual","refund","deferred","accrued"]);
				 
				 $scope.calculateFieldTotal(declared_income,["income_1","income_2","income_3","income_4","income_5","income_6","income_7","income_8","income_9","income_10","income_11","income_12"]);
				 
				 var bank_summary=$scope.formManager.fieldValues.bank_summary;
				 var cash_at_bank=$scope.formManager.fieldValues.cash_at_bank;
				 var bank_overdraft=$scope.formManager.fieldValues.bank_overdraft;
				 var bank_loan=$scope.formManager.fieldValues.bank_loan;
				 
				 for(var i in cash_at_bank)
				 {
					 cash_at_bank[i].difference.value =cash_at_bank[i].statement_balance.value -cash_at_bank[i].ledger_balance.value;
				 }
				 for(var i in bank_overdraft)
				 {
					 bank_overdraft[i].difference_od.value =bank_overdraft[i].statement_balance_od.value -bank_overdraft[i].ledger_balance_od.value;
				 }
				for(var i in bank_loan)
				 {
					 bank_loan[i].difference_loan.value =bank_loan[i].statement_balance_loan.value -bank_loan[i].ledger_balance_loan.value;
				 }

				 $scope.calculateColumnTotal(cash_at_bank,bank_summary,["statement_balance","ledger_balance","difference"])
				 $scope.calculateColumnTotal(bank_overdraft,bank_summary,["statement_balance_od","ledger_balance_od","difference_od"])
				 $scope.calculateColumnTotal(bank_loan,bank_summary,["statement_balance_loan","ledger_balance_loan","difference_loan"])
			 }
			 
			 $scope.formCalculations=function(index)
			 {
				 
			 }
			 
			 $scope.calculateColumnTotal=function(obj,total_obj,total_fields)
			 {
				 for(var f in total_fields)
				 {
					 var fldName=total_fields[f];
					// console.log(fldName);
					 total_obj[fldName].value=0;
				 }
				 for(var i in obj)
				 {
					 var row=obj[i];
					 ...
					 {
						 var fldName=total_fields[f];
						 total_obj[fldName].value+= row[fldName].value/1;
					 }
			 ...
			 $scope.onFormValueChange=function(fld)
			 {
				 ...
				 return $scope.calculateFormTotal();
	...
	$verficationCode=get_attribute_value($params,"verficationCode","");
			...
			$scope.createAuditDocument=function()
			{
				...
				.then(function(sr){
					...
					$scope.auditManager.currentAudit.engagementInfo.document_id=sr.lastInsertId;
					AuditEngagementService.save($scope.auditManager.currentAudit.engagementInfo);
					$scope.loadDocuments();
				});		
			}
		...
		function AuditPrintablesController($scope, $http, $filter,$stateParams,$state,$timeout,CoreAppSettingService,CoreLookupService,CoreLookupManager,AuditService)
		{
            ...
            {
                AuditService.readFolderContent("../files/company_files/official_docs")
                .then(function(sr){
                    var avaialableFiles=[];
                    
                    for(var i in sr.files)
                    {
                        avaialableFiles.push({name:sr.files[i].name}); 
                    }
                    $scope.officialDocs.available=avaialableFiles;
                    $scope.officialDocs.filesHash=buildHash(avaialableFiles,"f_","name");
                    for(var i in $scope.officialDocs.defaultSelected)
                        {
                           $scope.selectLicenseDoc($scope.officialDocs.defaultSelected[i]);
                        }
                 });
            }
			...
			$scope.initializePrintVariables=function()
			{
				...
				$scope.audit.form=$scope.formManager.formToObject($scope.formManager.fields);
				$scope.audit.accounts=$scope.accountsHash;
				console.log("initializePrintVariables",$scope.audit.form);
			}
