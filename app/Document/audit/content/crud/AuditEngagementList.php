<div class="padding" bm-ui-permission="app.crud.audit_engagement" login-user="app.loginUser">
	...
	<div class="box">
    	<div class="box-header {{audit_engagementManager.formData.list.headerStyle}}">
                    ...
                    <div class="dropdown-menu pull-right">
                        ...
                        <div class="dropdown-item dropdown-submenu">
                            ...
                            <div class="dropdown-menu pull-left" role="menu">
                                ...
                                <a class="dropdown-item" href=""><i class="fa  fa-file-word-o"></i> Word</a> <a class="dropdown-item" href=""><i class="fa  fa-file-pdf-o"></i> PDF</a>
                            </div>
                        </div>
                    </div>
                </li>
		...
		<table class="table m-b-none" ui-jp="footable" data-filter="#filter_AuditEngagement" data-page-size="5">
        <thead>
          <tr>
          	<td></td>
            <td style="width:150px;"></td> 
            
    <th >Customer Id</th><th >Schedules Date</th><th >Closing Date</th><th >Closing Date Ec</th><th >Audit Status</th><th >Archived</th>    
    </tr>
    </thead>
        <tbody>
    <tr ng-repeat="item in audit_engagementManager.audit_engagementsList track by $index">
    	
 		<td></td>
        <td>
         	<button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="audit_engagementManager.editItem(item.audit_engagement_id)">
                	<i class="fa fa-pencil"></i>
            </button>
        	
            <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="audit_engagementManager.deleteItem(item.audit_engagement_id)">
            		<i class="fa fa-trash-o"></i>
            </button>
        </td>

        <td>{{item.customer_id}}</td><td>{{item.schedules_date}}</td><td>{{item.closing_date}}</td><td>{{item.closing_date_ec}}</td><td>{{item.audit_status}}</td><td>{{item.archived}}</td>        
