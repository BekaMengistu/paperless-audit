(function(angular) {
	...
	.factory("FormBuilderManager",["FormBuilderService","DataImportExportService","CoreLookupService","$window",
			...
			.then(function()
				{
					...
					{
						return _formBuilderManager.formToObject(_formBuilderManager.fields);
					}
				}
			);
		}
		...
		{
			//if(fld.value_editor) {return fld.value_editor;}
			var defaultContainerTemplates={other:"default_fld.html",spacer:"spacer.html"}
			...
			var containerTemplate=defaultContainerTemplates[fld.data_type]
			
			
			
			fld.template.container=containerTemplate?containerTemplate : defaultContainerTemplates.other;
			
			fld.template.field=fldTemplate ? fldTemplate : defaultEditTemplates.other;
		...
		FormBuilderManager.prototype.setFieldTemplates=function(flds)
		{
			...
			for(var i in flds)
			{
				...
				if(fld.data_type=="object" && fld.repeating/1)
				{
				...
				{
					this.setSingleFieldTemplate(fld);
				}
		...
		/*
		FormBuilderManager.prototype.setFieldTemplates2=function(flds)
		{
