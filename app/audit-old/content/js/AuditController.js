(function() {
    ...
    angular
		...
		AuditFSController.$inject = ["$scope", "$http", "$filter","$stateParams","$state","$timeout","$locale","CoreAppSettingService","CoreLookupService","CoreLookupManager","AuditService","CRUDService","CRUDManagers"]
		

		AuditPrintablesController.$inject = ["$scope", "$http", "$filter","$stateParams","$state","$timeout","CoreAppSettingService","CoreLookupService","CoreLookupManager","AuditService","CRUDService"]
	...
	function AuditFormController($scope, $http, $filter,$stateParams,$state,CRUDService,CustomFormManager)
		{
             ...
             $scope.app.onFormPageLoad=function()
			 {
                ...
                $scope.activeForm=$scope.auditFormManager.forms[$scope.activeFormName];
                 ...
                 .then(function(sr){$scope.formInitialized=true;return sr;})
		...
		$scope.CapitalizeFirstLetters=function(text)
	{
		...
		$scope.calculateDerivedVars=function()
		{
            ...
            engagementInfo.year0Label =$filter('date')(engagementInfo.closing_date, "yyyy");
            
            engagementInfo.year1Label =engagementInfo.lastYearLabel=engagementInfo.year0Label-1
            engagementInfo.year2Label =engagementInfo.year0Label-2;
...
/*
        ...
        $scope.groupdTBToTable=function(sheetData,treeTable,sheetName,format)
		{
			...
			for(var i in treeTable)
			{
				...
				{
					var bt=acct[b];
                    for(var year_balance in {year0_balance:"",year1_balance:"",year2_balance:""})
		...
		$scope.buildRelations=function()
		{
            ...
            for(var i in $scope.auditManager.currentAudit.disclosureNotes)
            {
                ...
                $scope.addRelationship(target,"disclosureNotes",disclosure_note);
		...
		$scope.initializeTree=function()
			...
			$scope.reCalculateBalances=function()
			{
				...
				for(var i in $scope.treeManager.treeHash)
				{
                    ...
                    var balanceVars=["year0_balance","year1_balance","year2_balance","difference0","difference1"]
			...
			{
				var fields={debit:0,credit:0,current_balance:0,last_fye_balance:0,year2_balance:0
	...
	function AuditAdjustmentController($scope, $http, $filter,$stateParams,$state,$timeout,AuditService,AuditAdjustmentService)
	{
            ...
            {
                var adjustment1={audit_id:$scope.audit_engagement_id,account_id:"",debit:0,credit:0,prior_debit:0,priod_credit:0,year2_debit:0,year2_credit:0,justification:""};
            ...
            $scope.postYearEndTxns=function()
            {
                ...
                /*
                console.log({profit_before_tax:(profit_before_tax_acct.adjustedBalace.year0_balance *  profit_before_tax_acct.normal_balance)
                             ,disallowed_expenses:(disallowed_expenses_acct.adjustedBalace.year0_balance *  disallowed_expenses_acct.normal_balance)
                            ,income_taxed_at_source:(income_taxed_at_source_acct.adjustedBalace.year0_balance *  income_taxed_at_source_acct.normal_balance)
                            })
                
                
                var adjustment1={audit_id:$scope.audit_engagement_id,account_id:"PTExpense",debit:profit_tax,credit:0,prior_debit:0,priod_credit:0,year2_debit:0,year2_credit:0,justification:"Profit Tax"};
                
                var adjustment2={audit_id:$scope.audit_engagement_id,account_id:"PTPayable",debit:0,credit:profit_tax,prior_debit:0,priod_credit:0,year2_debit:0,year2_credit:0,justification:"Profit Tax"};
                */
                
                var adjustment1=$scope.createNewAdjustmentData({account_id:"PTExpense",debit:profit_tax,justification:"Profit Tax"});
                var adjustment2=$scope.createNewAdjustmentData({account_id:"PTPayable",credit:profit_tax,justification:"Profit Tax"});
                ...
                if(confirm("This will post end of year transactions \n Profit Tax=" + profitTaxFormatted +
                           ...
                           "\n Are you sure?"))
                {
                    $scope.postAdjustmentArray(adjustments,0);
                }
                /*
                if(confirm("This will post end of year transactions \n Profit Tax=" + profitTaxFormatted +
                           "\n Legal Reserve = " + legal_reserve + 
            ...
            $scope.postLegalReserveTxns=function(adjustments,profit_after_tax)
            {
                ...
                adjustments.push($scope.createNewAdjustmentData({account_id:"LRExpense",debit:lrExpense,justification:"Legal Reserve"}))
                adjustments.push($scope.createNewAdjustmentData({account_id:"LRPayable",credit:lrExpense,justification:"Legal Reserve"}))
               return lrExpense;
            }
...
/*--------------/SubstantiveAuditController------------------*/
	...
	$verficationCode=get_attribute_value($params,"verficationCode","");
		...
		function AuditPrintablesController($scope, $http, $filter,$stateParams,$state,$timeout,CoreAppSettingService,CoreLookupService,CoreLookupManager,AuditService,CRUDService)
		{
           ...
           // $scope.officialDocs.selectedFolder="";
            ...
            ,deliverables:{name:"deliverables",label:"Deliverables",onShow:$scope.readDeliverableTemplates,param:{header:"every-page",footer:"every-page",hideQr:false},footer:"footer-qr.html",config:{letter_template:true,header:0,footer:0}
					,contents:[
			...
			,financial_statements:{
					...
					,page:"app/Audit/content/printables/fs/fs.main-old.html"
		...
		function AuditFSController($scope, $http, $filter,$stateParams,$state,$timeout,$locale,CoreAppSettingService,CoreLookupService,CoreLookupManager,AuditService,CRUDService,CRUDManagers)
		{
            ...
            {
			var balance_sheet={name:"balance_sheet",title:"Statement of Financial Position",subTitle:$scope.audit.sfp_date,calculated:"{{2*34}}"
							,bs_items:[
                                
                                    {system_name:"assets",tree_depth:3,notes:"none"}
                                
                                   /* ,{system_name:"non_current_assets",tree_depth:2,notes:"none"}
                                    ,{system_name:"current_assets",tree_depth:2,notes:"none"}
                                */
                                    ,{system_name:"equity_and_liabilities",tree_depth:1,style:"no-number",note:"none"}
                                    ,{system_name:"capital_and_reserves",tree_depth:2,note:"none",level:2}
                                    ,{system_name:"total_liabilities",tree_depth:3,level:2}
                                    ,{system_name:"equity_and_liabilities",tree_depth:1,style:"total",note:"none"}
                                      ]}
            ...
            var income_statement={name:"income_statement"
							      ...
							      ,bs_items:[
                                      {system_name:"income",tree_depth:1,level:3}
                                    ,{system_name:"cost_of_sales",tree_depth:1,level:3}   
                                    ,{system_name:"gross_profit",tree_depth:1,note:"none",level:3}   
                                    ,{system_name:"other_income",tree_depth:1,level:3} 
                                    ,{system_name:"operating_profit",tree_depth:1,note:"none",level:3} 
                                    ,{system_name:"general_and_administrative_exp",tree_depth:1,level:3}
                                    ,{system_name:"profit_before_interest_and_tax",tree_depth:1,note:"none",level:3}
                                    ,{system_name:"financial_charges",tree_depth:1,level:3}
                                      
                                      ,{system_name:"profit_before_tax",tree_depth:1,note:"none",level:3}
                                      ,{system_name:"profit_tax_expense_current",tree_depth:1,level:3}
                                      ,{system_name:"profit_after_tax",tree_depth:1,note:"none",level:3}
                                      ,{system_name:"transferred_to_legal_reserve",tree_depth:1,note:"none",level:3}
                                      ,{system_name:"transferred_to_retained_earning",tree_depth:1,note:"none",level:3}
                                      
                                      
                                  ]
							}
                var cashFlowStatement={name:"cash_flow_st",title:"Statement of Cash Flow",bs_items:[{system_name:"cf_cash_flow",tree_depth:5}]}
            
            
            
			var cash_flow={name:"cash_flow",title:"Statement of Cash Flow",subTitle:$scope.audit.for_year_caps,templateUrl:"api/WebContentService/getContent/cash_flow_report"}
            
			var equity_change={name:"equity_change"
                               ,title:"Statement of Change in Equity"
                               ,subTitle:$scope.audit.for_year_caps
                               ,templateUrl:"api/WebContentService/getContent/statement_of_changes_in_equity_report"}
			...
			$scope.auditFS={
				...
				,notes:[
					...
					,{index:2,label:"Significant accounting policies"
                     ,textNote:[{templateUrl:"api/WebContentService/getContent/audit_note_acct_policies"}]}]
                //,notes:[]
				}
            $scope.auditFS.reports={balance_sheet:balance_sheet,income_statement:income_statement,cash_flow_st:cashFlowStatement,equity_change:equity_change};
			...
			{
                //console.log("-----Accounts    ",$scope.audit.accounts);
			...
			$scope.fsHelper.makeFSReports=function()
			{
				...
				$scope.fsHelper.calculateCashFlow();
            ...
            $scope.fsHelper.initializeCustomReports=function()
            {
                ...
                $scope.fsHelper.columnDefinitions=
                    {
                        account_label:{header:"Account",value:"report_label",template:"fs_cell_value.html"}
                        ,year0_balance:{header:"closing_date",value:"year0_balance",style:"number",template:"fs_balance_value.html"}
                        ,year1_balance:{header:"comparative_date",value:"year1_balance",style:"number",template:"fs_balance_value.html"}
                        ,year2_balance:{header:"comparative_date2",value:"year2_balance",style:"number",template:"fs_balance_value.html"}
                        ,difference0:{header:"Difference",headerLabel:"Difference",value:"difference0",style:"number",template:"fs_balance_value.html"}
                        ,difference1:{header:"Difference",headerLabel:"Difference",value:"difference1",style:"number",template:"fs_balance_value.html"}
            ...
            $scope.fsHelper.calculateCashFlow=function()
            {
                var accounts=$scope.audit.accounts;
                var temporary_accounts={
                    _profit_before_tax:{report_label:"Profit (loss) befor tax"
                                        ,year0_balance:accounts.profit_before_tax.data.adjustedBalance.year0_balance
                                        ,year1_balance:-12345678
                                       }
                   ,_change_in_deferred_taxes:{
                       year0_balance:accounts.opening_balance_difference.data.year0_balance
                       ,year1_balance:-12345678
                        }
                    ,_depreciation:{
                        year0_balance:accounts.prior_period_adjustment.data.adjustedBalance.year0_balance
                       ,year1_balance:-12345678}
                    ,_amortization:{
                        year0_balance:accounts.opening_balance_difference.data.year0_balance
                       ,year1_balance:-12345678
                        }
                    ,_net_cash_flow_before_captal:{
                        year0_balance:accounts.opening_balance_difference.data.year0_balance
                       ,year1_balance:-12345678
                    }
                }
                for(var i in temporary_accounts)
                {
                    var account=temporary_accounts[i];
                    $scope.audit.accounts[i]={data:account};
                }
                
                
               $scope.audit.accounts._profit_before_tax.data.year1_balance=-12345678; 
            }
            ...
            $scope.app.onFsLoad=function()
            {
                ...
                //$scope.app.initializePrintVariables()
                //.then($scope.fsHelper.makeFSReports);
            }
