...
<div class="padding" bm-ui-permission="app.crud.app_permission" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{app_permissionManager.formData.list.headerStyle}} non-select">
            <h2>{{app_permissionManager.formData.list.title}}</h2>
            <small>{{app_permissionManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_AppPermission" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="app_permissionManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_AppPermission" data-page-size="5">
                    <thead>
                        <tr>
...
<th >Parent</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in app_permissionManager.app_permissionsList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="app_permissionManager.editItem(item.app_permission_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="app_permissionManager.deleteItem(item.app_permission_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
                            <td>{{item.name}}</td>
