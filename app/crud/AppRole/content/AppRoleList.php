...
<div class="padding" bm-ui-permission="app.crud.app_role" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{app_roleManager.formData.list.headerStyle}} non-select">
            <h2>{{app_roleManager.formData.list.title}}</h2>
            <small>{{app_roleManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_AppRole" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="app_roleManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_AppRole" data-page-size="5">
                    <thead>
                        <tr>
...
<th >Description</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in app_roleManager.app_rolesList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="app_roleManager.editItem(item.app_role_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="app_roleManager.deleteItem(item.app_role_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
                            <td>{{item.name}}</td>
