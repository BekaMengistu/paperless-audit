...
<div class="padding" bm-ui-permission="app.crud.audit_adjustment" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{audit_adjustmentManager.formData.list.headerStyle}} non-select">
            <h2>{{audit_adjustmentManager.formData.list.title}}</h2>
            <small>{{audit_adjustmentManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_AuditAdjustment" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="audit_adjustmentManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_AuditAdjustment" data-page-size="5">
                    <thead>
                        <tr>
...
<th >Justification</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in audit_adjustmentManager.audit_adjustmentsList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="audit_adjustmentManager.editItem(item.audit_adjustment_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="audit_adjustmentManager.deleteItem(item.audit_adjustment_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
                            <td>{{item.audit_id}}</td>
<td>{{item.account_id}}</td>
<td>{{item.debit}}</td>
<td>{{item.credit}}</td>
<td>{{item.prior_debit}}</td>
<td>{{item.priod_credit}}</td>
