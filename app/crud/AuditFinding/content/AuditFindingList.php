...
<div class="padding" bm-ui-permission="app.crud.audit_finding" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{audit_findingManager.formData.list.headerStyle}} non-select">
            <h2>{{audit_findingManager.formData.list.title}}</h2>
            <small>{{audit_findingManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_AuditFinding" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="audit_findingManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_AuditFinding" data-page-size="5">
                    <thead>
                        <tr>
...
<th >User Id</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in audit_findingManager.audit_findingsList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="audit_findingManager.editItem(item.audit_finding_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="audit_findingManager.deleteItem(item.audit_finding_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
                            <td>{{item.audit_id}}</td>
