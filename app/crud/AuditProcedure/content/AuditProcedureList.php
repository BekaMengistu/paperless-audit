...
<div class="padding" bm-ui-permission="app.crud.audit_procedure" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{audit_procedureManager.formData.list.headerStyle}} non-select">
            <h2>{{audit_procedureManager.formData.list.title}}</h2>
            <small>{{audit_procedureManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_AuditProcedure" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="audit_procedureManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_AuditProcedure" data-page-size="5">
                    <thead>
                        <tr>
...
<th >Priority</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in audit_procedureManager.audit_proceduresList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="audit_procedureManager.editItem(item.audit_procedure_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="audit_procedureManager.deleteItem(item.audit_procedure_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
                            <td>{{item.ref}}</td>
