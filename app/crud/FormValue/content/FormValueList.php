...
<div class="padding" bm-ui-permission="app.crud.form_value" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{form_valueManager.formData.list.headerStyle}} non-select">
            <h2>{{form_valueManager.formData.list.title}}</h2>
            <small>{{form_valueManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_FormValue" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="form_valueManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_FormValue" data-page-size="5">
                    <thead>
                        <tr>
...
<th >Archived</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in form_valueManager.form_valuesList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="form_valueManager.editItem(item.form_value_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="form_valueManager.deleteItem(item.form_value_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
