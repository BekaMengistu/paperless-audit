...
<div class="padding" bm-ui-permission="app.crud.user_role" login-user="app.loginUser">
    <div ng-if="dataImportManager.windowVisible">
        <span ng-include="dataImportManager.importOptions.templatePath"></span>
    </div>
    <div class="box">
        <div class="box-header {{user_roleManager.formData.list.headerStyle}} non-select">
            <h2>{{user_roleManager.formData.list.title}}</h2>
            <small>{{user_roleManager.formData.list.subTitle}}</small>
        </div>
        <div class="box-tool">
            <ul class="nav" >
                <li class="nav-item inline">
                    <input id="filter_UserRole" type="text" placeholder="Search" class="form-control input-sm w-auto inline m-r"/>
                </li>
                <li class="nav-item inline">
                    <button uib-tooltip="New" ng-click="user_roleManager.createItem()" 
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table class="table m-b-none table-condenced non-select" ui-jp="footable" data-filter="#filter_UserRole" data-page-size="5">
                    <thead>
                        <tr>
...
<th >App Role Id</th>
                    ...
                    <tbody>
                        <tr class="hover-target hover-shadow" ng-repeat="item in user_roleManager.user_rolesList track by $index">
                            <td><span class="hover-show">
                                <button uib-tooltip="Edit" class="btn btn-info btn-xs"  
            	ng-click="user_roleManager.editItem(item.user_role_id)"> <i class="fa fa-pencil"></i> </button>
                                <button uib-tooltip="Delete" class="btn btn-danger  btn-xs" 
            	ng-click="user_roleManager.deleteItem(item.user_role_id)"> <i class="fa fa-trash-o"></i> </button>
                                </span></td>
                            <td>{{item.user_id}}</td>
