(function(angular) {
	...
	.service( "UserService", function( $q,$http )
	{
		return {
    		...
    		,changePassword : function(params){return excuteService($q,$http,"api/UserService/changePassword/", params,"post");}
	...
	.factory("UserManager",["UserService","DataImportManager",
		...
		UserManager.prototype.saveItem=function(item)
		{
			...
			.then( function( si )
			{
				if(si.user_id!=item.user_id)
				{
					_UserManager.editItem(si.user_id);
				}
                else
                {
                    for(var i in si)
        ...
        UserManager.prototype.loadNew=function(params)
		{
			var _UserManager=this;
			return this.newItem()
	 		.then(function(sr){_UserManager.editedUser=sr});
		}
		...
		UserManager.prototype.loadList=function()
		{
			...
			return this.readList()
	 		.then(function(sr){_userManager.usersList=sr; return sr;});
		}
