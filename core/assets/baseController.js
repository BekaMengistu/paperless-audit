...
function addLookups(lookups)
{
	for(var i in lookups)
		{
			...
			for(var j in lookups[i])
			{
				var lu=lookups[i][j];
				lu.module=i;
				lu.name=name;
				//applicationLookups[i+"." + j]=lu;
				applicationLookups[j]=lu;
			}
	...
	for(var i in lookups)
		{
			var moduleLookup=getAttributeValue(applicationLookups,i,{});
			for(var j in lookups[i])
			{
				moduleLookup[j]=lookups[i][j];
			}
			applicationLookups[i]=moduleLookup;
		}
}
...
function baseController($scope,$timeout,userService,AppService)
{
	...
	$scope.onPageLoad=function()
	{
		...
		$scope.appSettings.time=moment(new Date()).format()
		$scope.authenticateApp();
		userService.getLoginUser()
		.then(function(sr){
			$scope.app.loginUser=sr;
			//console.log(sr);
			} );
	}*/
    
    
    $scope.onApplicationLoad=function()
	{
		$scope.appSettings.time=moment(new Date()).format()
       // console.log("--------onApplicationLoad");
		return $scope.authenticateApp()
        .then(function(sr){return $scope.authenticateUser()})
	}
    $scope.handleRemoteRequests=function()
    {
        var request_id="";
        var saveResponseToRemote=function(response)
        {
            AppService.saveResponseToRemote({request_id:request_id,response:response});
        }
        
        AppService.pingMaster()
        .then(function(sr){
            if(sr.service)
            {
                request_id=sr.request_id;
                AppService.runApi(sr)
                .then(saveResponseToRemote)   
            }
        })
    }
    $scope.afterDataLoad=function(baseUrl)
    {
        //console.log("$scope.afterDataLoad",baseUrl);
        //$timeout(,3000);
        $timeout(function(){AppService.afterDataLoad({localPath:baseUrl});
                          // $scope.handleRemoteRequests()
                           },3000);
        ;
       // return AppService.pingMaster();
    }
    $scope.authenticateUser=function()
    {
		...
		.then(function(sr){
            ...
            if(!sr.user_id)
                {
                    $scope.app.executingPaused=true;
                }
			return sr;
			});
    }
	$scope.authenticateApp=function()
	{
		...
		.then(function(sr){
                $scope.app.authentication=sr;
                $scope.app.loadCount=sr.count;
                if(!sr.appAuthenticated)
                {
                    $scope.app.executingPaused=true;
                }
              //  console.log("----------------------authenticateApp",sr);
                return sr;
        } );
	}
    $scope.registerApp=function()
    {
       // console.log("registerApp");
        AppService.registerApp($scope.app.authentication.licenceKey)
        .then(function(sr){
            if(sr.appAuthenticated){$scope.app.resumeExecuteState();}
            else
                {
                   $scope.app.authentication.message="Invalid License Key"; 
                }
        })
	...
	$scope.signIn=function()
	{
		...
		.then(function(si) {
			$scope.app.user=si;
           // console.log("--signInresult",$scope.app.user, $scope.app.suspendedState);
			if(si.user_id)
			{
                $scope.app.loginUser=si;
                $scope.app.resumeExecuteState();
			}
		});
	}
